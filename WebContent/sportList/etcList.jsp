...
.etc_mainpic {
	...
	height: 200px;
	margin: 10px auto;
	background-image: url("../img/etc_one.jpg");
...
<script>
	...
	function writerSearch(i) {
		location.href = "/semi_Jungo/sportList/etcList.jsp?skey=writer&svalue="
				+ i;
	}
</script>
...
<%
	...
	String sid_etc = (String) session.getAttribute("sid");
...
<body>
				...
				<div id="pagetop">
						...
						<div id="history">
							<a class="home" href="/semi_Jungo/index.jsp" title="Home"><img alt="home" src="../img/home_s.png"></a> &gt; <a
								...
								href="/semi_Jungo/sportList/etcList.jsp">ETC</a>
						</div>
				</div>
				<div id="content">
	<div class="etc_mainpic"></div>
			<div class="sortSelect">
				<form style="text-align: right;" name="fob" action="soccerList.jsp">
 ...
 %> <a
							href="sportContent.jsp?idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>"><%=arr.get(i).getSubject()%></a>
							<%
						...
						<td align="center"><div id="tag">
								...
								<ul>
									...
									<%
										if (!(arr.get(i).getWriter().equals(sid_etc))) {
									%>
									<li><a
				...
				<tfoot>
					<tr>
						<td colspan="7" align="center">
							...
							%> <a
							href="etcList.jsp?cp=<%=(userGroup - 1) * pageSize + pageSize%>&orderby=<%=orderby%>">&lt;&lt;</a>
							<%
								}
								for (int i = (userGroup * pageSize + 1)/*시작수를 현재 페이지로*/; i <= (userGroup * pageSize + pageSize); i++) {
							%> &nbsp;&nbsp;<a
							href="etcList.jsp?cp=<%=i%>&orderby=<%=orderby%>"><%=i%></a>&nbsp;&nbsp;
							<%
								if (i == totalPage)
										break;
								}
							...
							%><a
							href="etcList.jsp?cp=<%=(userGroup + 1) * pageSize + 1%>&orderby=<%=orderby%>">&gt;&gt;</a>
							<%
								}
							%>
						</td>
