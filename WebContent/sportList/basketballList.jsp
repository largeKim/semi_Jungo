...
.basketball_mainpic{
	...
	height: 200px;
	margin: 10px auto;
	background-image: url("../img/basketball_one.jpg");
...
function writerSearch(i){
	location.href="/semi_Jungo/sportList/basketballList.jsp?skey=writer&svalue="+i;
}
</script>
...
<%
	...
	String sort = "basketball";
	String orderby = request.getParameter("orderby");
	if (orderby == null || orderby.equals("")) {
		orderby = "all";
	}
	
	String sid_basket = (String) session.getAttribute("sid");
...
<section>
				...
				<div id="pagetop">
						...
						<div id="history">
							<a class="home" href="/semi_Jungo/index.jsp" title="Home"><img alt="home" src="../img/home_s.png"></a> &gt; <a
								href="/semi_Jungo/productList/productSellList.jsp">스포츠/자유</a> &gt; <a
								href="/semi_Jungo/sportList/basketballList.jsp">Basketball</a>
						</div>
				</div>
				<div id="content">
		<div class="basketball_mainpic">
		</div>
			<div class="sortSelect">
			<form style="text-align: right;" name="fob" action="basketballList.jsp">
 ...
 [<%=scarr.size()%>]
						...
						<td align="center"><div id="tag"><a href="#"><%=arr.get(i).getWriter()%></a>
							<ul>
							
							<%
							if(!(arr.get(i).getWriter().equals(sid_basket))){
							%>
				...
				<tfoot>
					<tr>
						<td colspan="7" align="center">
							...
							%> <a
							href="basketballList.jsp?cp=<%=(userGroup - 1) * pageSize + pageSize%>&orderby=<%=orderby%>">&lt;&lt;</a>
							<%
								}
								for (int i = (userGroup * pageSize + 1)/*시작수를 현재 페이지로*/; i <= (userGroup * pageSize + pageSize); i++) {
							%> &nbsp;&nbsp;<a
							href="basketballList.jsp?cp=<%=i%>&orderby=<%=orderby%>"><%=i%></a>&nbsp;&nbsp;
							<%
								if (i == totalPage)
										break;
								}
							...
							%><a
							href="basketballList.jsp?cp=<%=(userGroup + 1) * pageSize + 1%>&orderby=<%=orderby%>">&gt;&gt;</a>
							<%
								}
							%>
						</td>
			...
			<div class="basketballList_Search">
