<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="tt.pointex.*"%>
<%@page import="java.io.*"%>
<jsp:useBean id="wf" class="tt.wf.WebFolderDAO" scope="session"></jsp:useBean>
<jsp:useBean id="podao" class="tt.pointex.PointexDAO" scope="page"></jsp:useBean>
<!DOCTYPE html>
<%
...
PointexDTO dto = podao.pointExContent(idx);
%>
<section>
	<article>
		<h2>글쓰기</h2>
		<form name="pointAdd" action="adminPointExUpdate_ok.jsp">
				<input type="hidden" name="idx" value="<%=dto.getIdx()%>">
			<table class="p_WriteTable">
				<tr>
					<th>상품 이름</th>
					<td class="Write_td2"><input type="text" name="pname" value="<%=dto.getPname()%>" required></td>
					<th>상품 수량</th>
					<td class="Write_td2"><input type="number" name="pnum" value="<%=dto.getPnum()%>" required></td>
				</tr>
				<tr>
					<th>상품코드</th>
					<td class="Write_td2"><input type="text" name="pcode" value="<%=dto.getPcode()%>" required></td>
					<th>가격</th>
					<td class="Write_td2"><input type="number" name="pprice" value="<%=dto.getPprice()%>" required></td>
				</tr>
				<tr>
					<th>사진 등록</th>
					...
					<input type="text" name="picpath" readonly="readonly" value="<%=dto.getPicpath()%>"></td>
				</tr>
			</table>
			<div class="wrbtList">
