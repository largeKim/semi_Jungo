...
function writerSearch(i){
	location.href="/semi_Jungo/productList/productBuyList.jsp?skey=writer&svalue="+i;
}
</script>
</head>
...
<%
	...
	String sid_buy = (String) session.getAttribute("sid");
				...
				<div id="pagetop">
						...
						<div id="history">
							<a class="home" href="/semi_Jungo/index.jsp" title="Home"><img alt="home" src="../img/home_s.png"></a> &gt; <a
								...
								href="/semi_Jungo/productList/productBuyList.jsp">Buy</a>
						</div>
				</div>
				<div id="content">
				<div class="buy_mainpic"></div>
			<div class="sortSelect">
				<form name="f_sort" action="productBuyList.jsp">
					<select name="sort">
					...
					</select> <input type="submit" value="찾기" class="pr_sel_bt">
				</form>
			</div>
...
// 추가 수정부분 170402 일 끝
					...
					<tr>
						...
						<td style="padding-left: 10px;"><a
							href="productContent.jsp?idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>"><%=arr.get(i).getSubject()%></a></td>
						<td><div id="tag"><a href="#"><%=arr.get(i).getWriter()%></a>
							<ul>
							
							<%
							if(!(arr.get(i).getWriter().equals(sid_buy))){
							%>
				...
				<tfoot>
					<tr>
						<td colspan="6" align="center" style="clear: both;">
							...
							%> <a
							href="productBuyList.jsp?cp=<%=(userGroup - 1) * pageSize + pageSize%>">&lt;&lt;</a>
							<%
								}
								for (int i = (userGroup * pageSize + 1)/*시작수를 현재 페이지로*/; i <= (userGroup * pageSize + pageSize); i++) {
							%> &nbsp;&nbsp;<a href="productBuyList.jsp?cp=<%=i%>"><%=i%></a>&nbsp;&nbsp;
							<%
								if (i == totalPage)
										break;
								}
							...
							%><a
							href="productBuyList.jsp?cp=<%=(userGroup + 1) * pageSize + 1%>">&gt;&gt;</a>
							<%
								}
							%>
						</td>
