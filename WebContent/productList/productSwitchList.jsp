...
function writerSearch(i){
	location.href="/semi_Jungo/productList/productSwitchList.jsp?skey=writer&svalue="+i;
}
</script>
</head>
...
String sid_switch = (String) session.getAttribute("sid");

int totalCnt = 0;
if(skey==null || svalue==null){
	
	totalCnt = pdao.gettotalCnt(type);// 총 게시물수
	
}else{
	
	totalCnt = pdao.gettotalCnt(type, skey, svalue);
				...
				<div id="pagetop">
						...
						<div id="history">
							<a class="home" href="/semi_Jungo/index.jsp" title="Home"><img alt="home" src="../img/home_s.png"></a> &gt; <a
								...
								href="/semi_Jungo/productList/productSwitchList.jsp">Exchange</a>
						</div>
				</div>
				<div id="content">
				<div class="switch_mainpic"></div>
		<div class="sortSelect">
			<form name="f_sort" action="productSwitchList.jsp">
				<select name="sort">
			...
			if(arr==null||arr.size()==0){
				...
				for(int i = 0; i<arr.size(); i++){
					...
					<tr>
						...
						<td>
						<a href="productContent.jsp?idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>"><%=arr.get(i).getSubject() %></a></td>
						<td><div id="tag"><a href="#"><%=arr.get(i).getWriter()%></a>
							<ul>
							
							<%
							if(!(arr.get(i).getWriter().equals(sid_switch))){
							%>
			...
			<tfoot>
				<tr>
					...
					if(userGroup!=0){ // user그룹이 첫번째가 아니면
						%>
						<a href="productSwitchList.jsp?cp=<%=(userGroup-1)*pageSize+pageSize %>">&lt;&lt;</a>
						<%
					}
					for(int i = (userGroup*pageSize+1)/*시작수를 현재 페이지로*/; i<=(userGroup*pageSize+pageSize);i++){
						
						%>
						&nbsp;&nbsp;<a href="productSwitchList.jsp?cp=<%=i%>"><%=i%></a>&nbsp;&nbsp;
						<%
						if(i==totalPage)break;
					}
					
					if(userGroup!=(totalPage/pageSize-(totalPage%pageSize==0?1:0))){ // 유저그룹이 마지막페이지가 아니면
						%><a href="productSwitchList.jsp?cp=<%=(userGroup+1)*pageSize+1 %>">&gt;&gt;</a><%
					}
					%>
					</td>
				</tr>
