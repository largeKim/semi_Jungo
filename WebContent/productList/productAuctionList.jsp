				...
				<div id="pagetop">
						...
						<div id="history">
							<a class="home" href="/semi_Jungo/index.jsp" title="Home"><img alt="home" src="../img/home_s.png"></a> &gt; <a
								...
								href="/semi_Jungo/productList/productAuctionList.jsp">Auction</a>
						</div>
				</div>
				<div id="content">
				<div class="auction_mainpic"></div>
			<div class="sortSelect">
				<form name="f_sort" action="productAuctionList.jsp">
					<select name="sort">
...
// 추가 수정부분 170402 일 끝
				...
				%>
					...
					<td style="padding-left: 10px;">
 					...
 					%>
						<a href="auctionContent.jsp?idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>"><%=arr.get(i).getSubject().length()>20?arr.get(i).getSubject().substring(0,20)+"…..":arr.get(i).getSubject()%></a></td>
				...
				<tfoot>
					<tr>
						<td colspan="6" align="center" style="clear: both;">
							...
							%> <a
							href="productAuctionList.jsp?cp=<%=(userGroup - 1) * pageSize + pageSize%>">&lt;&lt;</a>
							<%
								}
								for (int i = (userGroup * pageSize + 1)/*시작수를 현재 페이지로*/; i <= (userGroup * pageSize + pageSize); i++) {
							%> &nbsp;&nbsp;<a href="productAuctionList.jsp?cp=<%=i%>"><%=i%></a>&nbsp;&nbsp;
							<%
								if (i == totalPage)
										break;
								}
							...
							%><a
							href="productAuctionList.jsp?cp=<%=(userGroup + 1) * pageSize + 1%>">&gt;&gt;</a>
							<%
								}
							%>
						</td>
			...
			<div class="pr_sell_bt">
				<a href="auctionWrite.jsp?type=<%=type%>">글쓰기</a>
			</div>
			</div>
			</div>
		</div>
